...
class Main implements InterfaceTextures, InterfaceColors {
  ...
  private void addAllToTG(TransformGroup tg) {
    Planet mars = new Planet(1.0f, 1.0f, TEXTURE_MARS, this.defaultBound);
    tg.addChild(mars);

    TransformGroup tgNavicella = new TransformGroup();
    tgNavicella.addChild(new Navicella(1.0f));
    Transform3D tNavicella = new Transform3D();
    tNavicella.setTranslation(new Vector3d(-2.0f, 0.0f, 0.0f));
    tgNavicella.setTransform(tNavicella);
    tg.addChild(tgNavicella);

    TransformGroup tgNavicellaCubo = new TransformGroup();
    tgNavicellaCubo.addChild(new NavicellaCubo(1.0f));
    Transform3D tNavicellaCubo = new Transform3D();
    tNavicellaCubo.setTranslation(new Vector3d(+2.0f, 0.0f, 0.0f));
    tgNavicellaCubo.setTransform(tNavicellaCubo);
    tg.addChild(tgNavicellaCubo);
  }
